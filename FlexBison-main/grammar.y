...
checkedButton:       T_ANDROID_CHECKEDBUTTON T_STRING
                    ...
                    printf("%s = %s\n", $1, $2);
                    if(isIdUsed($2)){
                        yyerror("The android:checkedButton value is not unique.");
                    }else{
                        addUsedId($2);
                        checkRadioButtonId($2); // check if checkedButton id is in the list
                    }
                    } 
                    ; */
...
radiogroup:               T_RADIO_GROUP radio_start radiogroupattributes T_END_TAG radiobutton radio_end T_END_RADIO_GROUP   //{checkRadioBId();}; //<RadioGroup radiogroupattributes> radiobutton </Radiogroup>
                        ; 
...
/* radiogroup:               T_RADIO_GROUP   radiogroupattributes T_END_TAG radiobutton T_END_RADIO_GROUP { checkRadioButtonId(); checked_button_id = NULL; radio_button_id_count = 0;}  //<RadioGroup radiogroupattributes> radiobutton </Radiogroup>
                        ;                        */
